...
SELECT K.PARTEI
FROM (AGGREGATKANDIDATERSTSTIMMEN A JOIN KANDIDATENSIEGERPUNKTE S ON ((A.STIMMKREISNR = S.STIMMKREISNR) AND (S.SIEGERPUNKTE = A.ANZAHLERSTSTIMMEN) AND (A.JAHR = S.JAHR))) JOIN KANDIDAT K ON ((K.ID = A.KANDIDATNR) AND (K.JAHR = A.JAHR))
